...
def apply_review(data: dict, report: dict, force_all: bool = False) -> tuple[dict, list[str], list[str]]:
    ...
    for entry in data["items"]:
        ...
        if not force_all and (not all_exact or result["exceptions"]):
...
def main() -> None:
    ...
    p.add_argument("--force-all", action="store_true", help="例外・差分がある議案も含め残り全件をverifiedにする")
    args = p.parse_args()

    data = json.loads(args.data.read_text(encoding="utf-8"))
    report = json.loads(args.report.read_text(encoding="utf-8"))

    updated, verified_ids, skipped_ids = apply_review(data, report, args.force_all)
    ...
    report["humanReview"] = {
        ...
        "method": (
            "例外・差分がある議案を含め、残り全件を一括でverifiedにした（個別の目視照合は行っていない）"
            if args.force_all else
            "全52件の機械照合結果（officialNumber/title/officialSummary/resultの4項目exact一致）を人が一覧確認し、例外一覧の妥当性を検証したうえで一括承認"
        ),
